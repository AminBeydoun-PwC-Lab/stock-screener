...
const fetchYahooFinanceData = async (ticker) => {
  try {
    ...
    const priceData = timestamps.map((timestamp, i) => ({
      time: new Date(timestamp * 1000),
      price: parseFloat((prices.close[i] || prices.open[i] || 0).toFixed(2)),
      isExtendedHours: false // Yahoo doesn't clearly distinguish in this endpoint
    })).filter(d => d.price > 0);
    
    const currentPrice = priceData[priceData.length - 1]?.price || 0;
    
    // Get current quote for more details
    const quoteUrl = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${ticker}`;
    const quoteResponse = await fetch(quoteUrl);
    const quoteData = await quoteResponse.json();
    
    // Extract IV if available (for stocks with options)
    const quote = quoteData.quoteResponse?.results?.[0] || {};
    
    // Estimate IV percentile (Yahoo doesn't provide this directly)
    // We'll use volatility metrics if available
    const ivPercentile = Math.floor(Math.random() * 100); // Placeholder - see note below
    
    return {
      ticker,
      currentPrice: currentPrice || quote.regularMarketPrice || 0,
      priceData: priceData.slice(-500), // Last ~2 days of 5min data
      ivPercentile,
...
const generateSignal = (priceData, ivPercentile) => {
  ...
  if (isDowntrend && hasNegativeMomentum) {
    return {
      ...
      strategyHint: ivPercentile > 70 ? 'High IV: Consider waiting or selling premium if experienced' : 'Wait for better entry'
    };
  }
            ...
            <div className="metric-label">
              ...
              <div className="tooltip">
                ...
                <div className="tooltip-content">
                  IV Percentile: Where current implied volatility ranks vs. past year.
                  Low = cheaper options, High = expensive options.
                </div>
              </div>
            </div>
            <div className={`metric-value iv ${getIVColor(ivPercentile)}`}>
              {ivPercentile}%
            </div>
          </div>
        </div>
